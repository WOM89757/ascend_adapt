...
namespace {
    ...
    const uint32_t YOLO_RESIZE = 640;
}
...
{
	std::string yolov3ModelPath = "./model/yolov5m_bs1-1.om";
...
APP_ERROR YoloPostProcess(ImageInfo imageInfo, std::string& yoloV3ConfigPath, std::string& yoloV3LablePath,
                        std::vector<MxBase::Tensor>& yoloV3Outputs)
{
    ...
    imgInfo.widthResize = YOLO_RESIZE;
    imgInfo.heightResize = YOLO_RESIZE;
...
void ConductYoloInfer(int32_t deviceId, std::string modelPath, std::string imgPath, V2Param v2Param)
{
    APP_ERROR ret;
    // imageProcess init
    ...
    // model init
    MxBase::Model yolo(modelPath, deviceId);
    ...
    // set size param
    Size resizeConfig(YOLO_RESIZE, YOLO_RESIZE);
    ...
    // save resize image
    std::string path = "./resized_yolo_640.jpg";
    ...
    // make infer input
    std::vector<Tensor> yoloInputs = {tensorImg};
    // do infer
    std::vector<Tensor> yoloOutputs = yolo.Infer(yoloInputs);
    std::cout << "yoloOutputs len=" << yoloOutputs.size() << std::endl;

    // !move result to host!
    for (auto output : yoloOutputs) {
        output.ToHost();
    }

    // *****5模型后处理
    ImageInfo imageInfo;
    imageInfo.oriImagePath = imgPath;
    imageInfo.oriImage = decodedImage;
    
    ret = YoloPostProcess(imageInfo, v2Param.configPath, v2Param.labelPath, yoloOutputs);
    if (ret != APP_ERR_OK) {
...
int main(int argc, char* argv[])
{
    ...
    ConductYoloInfer(deviceId, modelPath, imgPath, v2Param);
