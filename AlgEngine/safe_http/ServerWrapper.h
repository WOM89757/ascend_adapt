...
public:
    ...
    void add_get_route(const std::string &path,
                       const std::function<void(http_request)> &handler);
    void add_post_route(const std::string &path,
                        const std::function<void(http_request)> &handler);
    void add_delete_route(const std::string &path,
                          const std::function<void(http_request)> &handler);
    void start();
    void stop();
...
private:
    ...
    void handle_exception(http_request request,
                          const std::string &errorMessage);
    const std::map<std::string, std::function<void(http_request)>> &
    get_route_map(const method &httpMethod);
