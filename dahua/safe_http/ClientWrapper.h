...
public:
    ...
    // GET 请求封装
    void get(const std::string& path, void* customInfo = nullptr,
             const std::function<void(json::value, void*)>& onSuccess =
                 default_on_success,
             const std::function<void(const std::string&)>& onError =
                 default_on_error);

    void post(const std::string& path, const json::value& body,
              void* customInfo = nullptr,
              const std::function<void(json::value, void*)>& onSuccess =
                  default_on_success,
              const std::function<void(const std::string&)>& onError =
    ...
    // 默认成功回调函数
    static void default_on_success(json::value response,
                                   void* customInfo = nullptr);
    // 默认错误回调函数
